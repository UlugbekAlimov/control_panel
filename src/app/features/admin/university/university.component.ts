...
export class UniversityComponent implements OnInit {
  // Экспортируем EducationType для использования в шаблоне
  ...
  selectedType = signal<EducationType>(EducationType.UNIVERSITY);
  ...
  columns: TableColumn[] = [
    ...
    { key: 'educationType', label: 'Тип', sortable: true },
  ...
  educationTypeOptions: SelectOption[] = [
    { value: EducationType.UNIVERSITY, label: 'ВУЗ' },
    { value: EducationType.COLLEGE, label: 'Колледж' },
    { value: EducationType.ABROAD, label: 'За границей' }
  ];
  ...
  loadData(): void {
    ...
    this.educationRecords.set(
      allRecords.filter(r =>
        r.educationType === EducationType.UNIVERSITY ||
        r.educationType === EducationType.COLLEGE ||
        r.educationType === EducationType.ABROAD
      )
    );
  }

  filterByType(type: EducationType): void {
    this.selectedType.set(type);
    const allRecords = this.citizenService.getEducationRecords();
    if (type === EducationType.UNIVERSITY) {
      this.educationRecords.set(
        allRecords.filter(r =>
          r.educationType === EducationType.UNIVERSITY ||
          r.educationType === EducationType.COLLEGE
      ...
      this.educationRecords.set(allRecords.filter(r => r.educationType === type));
    }
  }
  ...
  openAddModal(): void {
    ...
    this.formData = {
      citizenId: '',
      educationType: this.selectedType(),
      institutionName: '',
      startDate: '',
      endDate: '',
      isActive: true,
  ...
  expelStudent(record: EducationRecord): void {
    if (confirm('Зафиксировать отчисление? Военкомат будет автоматически уведомлен о снятии отсрочки.')) {
